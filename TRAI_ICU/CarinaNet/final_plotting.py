...
def _compute_spacing(both_annot, carinaNet_summary):
    ...
    for index in both_annot:
        if index in carinaNet_summary : #probe_loc_summary and index in carina_loc_summary:
            detected_spacing = carinaNet_summary[index]['CARINA']['pred'][1] - carinaNet_summary[index]['ETT']['pred'][1]
            GT_spacing = carinaNet_summary[index]['CARINA']['GT'][1] - carinaNet_summary[index]['ETT']['GT'][1]
...
def high_confidence_error(carinaNet_summary, err_dic, conf = 0.3):
    ...
    meanerr_ett = np.mean(np.abs(list(carina_err_conf.values())))
    meanerr_carina = np.mean(np.abs(list(ETT_err_conf.values())))
    ...
    print(f"Error on confidence > {conf:.2f} : {len(dist_err_conf)} images\n"
          ...
          f" ETT    : {meanerr_ett:.2f}cm std : {stderr_ett:.2f}\n"
          f" Carina : {meanerr_carina:.2f}cm std {stderr_carina:.2f}\n\n")
